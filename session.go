...
func (s *session) sendInReplyTo(msg *Message, inReplyTo *Message) error {
	...
	s.toSend = append(s.toSend, msgBytes)
	s.sendQueuedWithTimings(true, &sendTimings)
	s.sendMutex.Unlock()
	observeSendStages(sendTimings)
...
func (s *session) sendQueued(blockUntilSent bool) {
	s.sendQueuedWithTimings(blockUntilSent, nil)
}

func (s *session) sendQueuedWithTimings(blockUntilSent bool, timings *[]SendStageEvent) {
...
func (s *session) sendBytes(msg []byte, blockUntilSent bool, timings *[]SendStageEvent) bool {
	...
	if blockUntilSent {
		...
		s.messageOut <- msg
		s.recordSendStage(timings, "outbound_channel_wait", fixMsgTypeFromRaw(msg), time.Since(channelStartedAt), true)
	...
	case s.messageOut <- msg:
		...
		return true
	default:
		s.recordSendStage(timings, "outbound_channel_wait", fixMsgTypeFromRaw(msg), time.Since(channelStartedAt), false)
		return false
	}
}
...
func (s *session) recordSendStage(timings *[]SendStageEvent, stage, msgType string, duration time.Duration, success bool) {
	event := s.sendStageEvent(stage, msgType, duration, success)
	if timings == nil {
